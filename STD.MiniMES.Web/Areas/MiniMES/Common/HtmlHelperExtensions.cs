...
namespace System.Web.Mvc.Html
{
    public static class HtmlHelperExtensions
    {
        ...
        public static MvcHtmlString HideColumn(this HtmlHelper htmlHelper, List<dynamic> cols, string field)//public static MvcHtmlString HideColumn(this HtmlHelper htmlHelper,List<sys_roleMenuColumnMap> cols, string field)
        {
            var result = string.Empty;
            var fields = cols.Where(x => x.FieldName == field);
            if (fields.Count() > 0 && fields.First().IsReject == "1")
                result = "hidden=true";
