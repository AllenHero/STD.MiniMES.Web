...
var viewModel = function () {
    ...
    this.data = {
        ...
        database: ko.observableArray(), // [{ text: 'STD.EPS.Mms', value: 'Mms' }, { text: 'STD.EPS.Sys', value: 'Sys' }],
    ...
    this.initDatabase = function () {
        com.ajax({
            ...
            url: '/api/MiniMES/generator/GetConnectionStrings',
            success: function (d) {
    ...
    this.getTableUrl = function () {
        return '/api/MiniMES/generator/GetTables?database=' + self.form.database();
    };
    this.getColumnUrl = function (table) {
        return '/api/MiniMES/generator/GetColumns?database=' + self.form.database() + "&table=" + table;
    }
    ...
    this.codetype = {
        ...
        onSelect: function (node) {
            ...
            self.initWizard();
            //self.searchEdit.columntree2.url = ko.computed(function (){return '/api/MiniMES/generator/GetColumns?database=' + self.form.database() + '&table=' + self.form.table();})
        }
    };
    ...
    this.database = {
        ...
        onSelect: function (node) {
            ...
            //    self.tabletree.url('/api/MiniMES/generator/GetTables?database=' + self.form.database);
    ...
    this.tabletree = {
        ...
        onSelect: function (node) {
            ...
            //self.searchEdit.columntree.url('/api/MiniMES/generator/GetColumns?database=' + self.form.database + '&table=' + self.form.table);
            //self.searchEdit.columntree2.url('/api/MiniMES/generator/GetColumns?database=' + self.form.database + '&table=' + self.form.table);
    ...
    this.searchEdit.columntree = {
        ...
        checkbox: true,
        loadFilter: function (d) {
            return utils.filterProperties(d.rows || d, ['ColumnName as id', 'Description as text']);
        },
        onSelect: function (node) {
        ...
        onCheck: function (node, checked) {
            if (checked)
                self.form.conditions.push({ field: node.id, title: node.text, type: 'text', options: '', cp: 'equal',readonly:false });
            else
    ...
    this.searchEdit.columntree2 = {
        ...
        onCheck: function (node, checked) {
            ...
            if (checked) {
                ...
                item || arr.push({ field: node.id, title: node.text, hidden: false, sortable: true, align: 'left', width: 80, formatter: '', editor: 'text' });
            } else
    ...
    this.edit.columntree2 = {
        ...
        loadFilter: function (d) {
            self.data.column(d);
            var list = utils.filterProperties(d.rows || d, ['ColumnName as id', 'Description as text']);
        ...
        onCheck: function (node, checked) {
            ...
            if (checked) {
                var item = $.grep(arr(), function (row) { return row.field == node.id; })[0];
                item || arr.push({ field: node.id, title: node.text, hidden: false, sortable: true, align: 'left', width: 80, formatter: '', editor: 'text', type: '', readonly: true });
            } else
                arr.remove(function (item) { return item.field == node.id });
        }
    }
