...
easyuifix.easyloader_addLoadModules = function (modules) {
    $.extend(modules, {
        ...
        datetimebox: {
            js: "/Content/js/jquery-easyui-1.3.1/plugins/jquery.datetimebox.js",
            dependencies: ["combo"]
        },
        autocomplete: {
...
easyuifix.parser_addplugins = function (plugins) {
    var arr = ["daterange", "lookup", "autocomplete", "datetimebox"];
    for (var i in arr) 
        plugins.push(arr[i]);
};
...
easyuifix.onLoadSuccess = function (jq, rowIndex, row) {
    var opts = $.data(jq, "datagrid").options;
