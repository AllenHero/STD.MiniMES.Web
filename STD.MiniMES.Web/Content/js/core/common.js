...
com.formatCheckbox = function (value) {
    var checked = ((value=="1") || 'false').toString() == 'true';
...
com.exporter = function (opt) {
    ...
    initFromGrid = function (grid) {
        ...
        var columns = '';
        for (var i = 0; i < length; i++) {
            columns = options.columns[i];
            for (var j = 1; j < columns.length; j++) {
                var log = columns[j].field.toString().substring(columns[j].field.length - 2);
                if (log == 'Id' || log == 'id' || log == 'ID')
                {
                    if (columns[j].field.toString() == "LineStationId") {
                        columns[j].field = "StationName"
                    }
                    else {
                        columns[j].field = columns[j].field.substring(0, (columns[j].field.length - 2)) + 'Name';
                    }
                }
            }

            titles[i] = (options.frozenColumns[i] || []).concat(columns || [])
        }
