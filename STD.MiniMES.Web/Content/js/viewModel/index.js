...
wrapper.init = function () {
    com.ajax({ type: 'GET', url: '/api/MiniMES/menu', success: wrapper.initMenu });
    ...
    com.dialog({
        ...
        viewModel: function (w) {
		    ...
		    w.find("#pwd_confirm").click(function () {
                if (w.find('#oldpassword').val().length == 0)
                {
                    $.messager.confirm("系统提示", '必须输入原始密码！');
                    return;
                }
                if (w.find('#newpassword1').val().length == 0) {
                    $.messager.confirm("系统提示", '必须输入新密码！');
                    return;
                }
                if (w.find('#newpassword1').val() != w.find('#newpassword2').val())
                {
                    $.messager.confirm("系统提示", '新密码两次输入不一致！');
                    return;
                }
                com.ajax({
                    url: 'api/sys/user/PostChangePassword',
                    type:'post',
                    data: JSON.stringify({
                        usercode: w.find("[name=UserCode]").val(),
                        oldpassword: w.find('#oldpassword').val(),
                        newpassword: w.find('#newpassword1').val()
                    }),
                    success: function (d) {
                        if (d.status)
                            w.dialog('close');
...
wrapper.menuAccordion = function (menus) {
    ...
    $.each(menus, function () {
        var html = '<ul>';
        var temple = '<li><div><a ref="{0}" href="javascript:void(0)" rel="{1}" isNewPage="{4}"><span class="icon {2}">&nbsp;</span><span class="nav">{3}</span></a></div></li>';
        $.each(this.children || [], function () {
            html += utils.formatString(temple, this.MenuCode, this.URL, this.IconClass, this.MenuName, this.IsNewPage);
        });
        html += '</ul>';
    ...
    $obj.find('li').click(function () {
        ...
        var isNewPage = link.attr("isNewPage");

        if (isNewPage.toLowerCase() == "true") {
            window.open(url);
        }
        else {
